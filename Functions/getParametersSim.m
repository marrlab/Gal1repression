function getParametersSim(model,plt,server,pre_sol)
...
for imodel = model
    ...
    for icell = 1:size(simData,1)
        
        %display which total GFP trace is currently fitted
        disp(sprintf('%d of %d and model %d', icell,size(simData,1),imodel))
        
        %create dat structure for optimization
        DA(1).y = data(icell,:)';
        DA(1).t = (1-1)*3/60:3/60:(size(simData,2)-1)*3/60;
        ...
        if pre_sol == 0 %if do not load pre-computed parameters
            ...
            load(sprintf('./Results/scR1_model%d_sim',istrain,irep,imodel))
        end
    ...
    if pre_sol == 0
        ...
        %                 end
        save(sprintf('./Results/scR1_model%d_sim',imodel),'scR')
    end
